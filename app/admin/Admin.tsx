...
const Admin = () => {
   ...
   const handleOpenTambah = () => setOpenTambah(!open);
               ...
               <div className="flex-col min-h-screen  bg-white rounded-md drop-shadow-sm overflow-y-auto w-full  md:w-6/12 border-1 p-4 md:p-6">
                  ...
                  <ModalTambah
                     onConfrim={async () => {
                        try {
                           setLoading(true);
                           const { data, total_seluruh, min, max } =
                              await getData({
                                 page: 1,
                                 from: null,
                                 to: null,
                           ...
                           let maxYear = new Date(max);
                           let loop =
                              Number(maxYear.getFullYear()) -
                              Number(minYear.getFullYear());
                           let temp = [];
                           for (let i = 0; i <= loop; i++) {
                              temp.push(Number(minYear.getFullYear()) + i);
                           }
                     ...
                     open={openTambah}
                     handleOpen={() => setOpenTambah(false)}
                  />
                  ...
                  <div className="flex-row flex-wrap justify-center items-center flex gap-2 items-center bg-white rounded-md drop-shadow-sm w-full mt-4 md:w-full border-1 p-4 md:p-6">
                     {data?.links.map((el, idx) => (
                        <button
                           className={`${
                              el.active ? "bg-blue-200" : "bg-gray-300"
                           } ${Number(el.label) ? "w-8 py-1" : "p-1"} rounded `}
                           onClick={() => {
                              if (el.url !== null) {
                                 window.scrollTo({
                                    top: 0,
